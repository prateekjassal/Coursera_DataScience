...
def mapper(record):
    ...
    # Counting 4 card straights, now the values J, Q, K, A are also possible so increased the for loop range !!      
    for i in range(0, 11):
       if i==0 and is_straight == True:
        break; 
       if (counts[straightrunfaces[i]] and 
             counts[straightrunfaces[i+1]] and 
             counts[straightrunfaces[i+2]] and 
    ...
    elif is_4straight and not((is_trip and is_pair) or (is_quad) or(is_straight) or (is_flush)):
